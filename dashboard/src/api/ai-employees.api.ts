...
import type { AiEmployee, Department } from '../config/ai-employees.config';
import { AI_DEPARTMENTS, matchAbilityByWorkflow } from '../config/ai-employees.config';
  ...
  try {
    ...
    for (const dept of AI_DEPARTMENTS) {
      for (const employee of dept.employees) {
        ...
        stats.successRate =
          completedTasks > 0 ? Math.round((stats.todaySuccess / completedTasks) * 100) : 0;
