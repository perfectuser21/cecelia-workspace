  ...
  if (instanceLoading) {
    return (
      ...
      >
        <div className="text-center">
          <div
            className={`w-12 h-12 border-4 ${isCore ? 'border-slate-400' : 'border-blue-500'} border-t-transparent rounded-full animate-spin mx-auto mb-4`}
          />
          <p className={isCore ? 'text-slate-400' : 'text-gray-500 dark:text-gray-400'}>
            加载中...
          </p>
        </div>
      </div>
    ...
    >
      ...
      {isAuthenticated && (
        <>
          {/* 左侧导航栏 - 使用配置的渐变色 */}
          <aside
            className={`fixed inset-y-0 left-0 ${collapsed ? 'w-16' : 'w-64'} flex flex-col shadow-2xl transition-all duration-300 z-20`}
            style={{ background: config?.theme.sidebarGradient || 'var(--sidebar-gradient)' }}
          >
            {/* Logo 区域 - 从配置读取 */}
            <div
              className={`h-16 flex items-center ${collapsed ? 'justify-center' : 'justify-start'} px-4 border-b border-white/10`}
            >
              {collapsed ? (
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Icon
                            className={`w-5 h-5 ${collapsed ? '' : 'mr-3'} transition-transform duration-200 ${
                              isActive
                                ? isCore
                                  ? 'text-slate-300'
                                  : 'text-sky-300'
          ...
          {/* 顶部栏 - Canvas 页面使用深色主题 */}
          <div
            className={`fixed top-0 ${collapsed ? 'left-16' : 'left-64'} right-0 h-16 ${
      ...
      <main
        className={
          isAuthenticated
            ? `${collapsed ? 'ml-16' : 'ml-64'} pt-16 transition-all duration-300`
            : ''
        }
      >
        <div
          ...
          style={
            [
              ...
              ? { position: 'absolute', top: 64, left: collapsed ? 64 : 256, right: 0, bottom: 0 }
              : {}
          }
        >
          <DynamicRouter>
