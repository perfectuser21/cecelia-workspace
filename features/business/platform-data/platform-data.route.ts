...
router.get('/grouped', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const formStats: Record<string, { count: number; total_views: number; total_likes: number }> = {
    ...
    const dataWithMarks = result.rows.map((row: any) => ({
      ...row,
    ...
    logger.error('Failed to fetch grouped content', { error: error.message });
    next(error);
  }
});

// Get metrics history for a content item (详细数据)
router.get('/:contentId/metrics', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { contentId } = req.params;

    const result = await db.query(
      `SELECT * FROM content_metrics WHERE content_id = $1 ORDER BY days_after_publish`,
      [contentId]
    );
    ...
    logger.error('Failed to fetch content metrics', { error: error.message });
    next(error);
  }
});
...
router.post('/batch', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info('Batch inserting content data', { count: items.length });
    ...
    for (const item of items) {
      ...
      const nearestDay = targetDays.reduce((prev, curr) =>
        Math.abs(curr - daysAfterPublish) < Math.abs(prev - daysAfterPublish) ? curr : prev
      );
    ...
    logger.info('Batch insert completed', { inserted, updated, metricsInserted });
    ...
    logger.error('Failed to batch insert content data', { error: error.message });
    next(error);
  }
});
