...
import {
  ...
  ServicesResponse,
  ServiceStatus,
} from './vps-monitor.types';
import logger from '../../../shared/utils/logger';
import { vpsMonitorRepository } from './vps-monitor.repository';
...
class VPSMonitorService {
  ...
  async getDockerStats(): Promise<DockerStats> {
    try {
      ...
      try {
        ...
        logger.warn('Failed to get docker stats (non-critical)', { error: statsError });
      }
   ...
   */
  async getServicesStatus(): Promise<ServicesResponse> {
    try {
      const dockerStats = await this.getDockerStats();
      const containerMap = new Map(
        dockerStats.containers.map(c => [c.name, c])
      );

      const services: ServiceStatus[] = this.KEY_SERVICES.map(service => {
  ...
  async saveMetrics(): Promise<void> {
    try {
      ...
      await vpsMonitorRepository.saveMetric({
      ...
      logger.debug('VPS metrics saved successfully');
    } catch (error) {
  ...
  async getMetricsHistory(hours: number = 24): Promise<any[]> {
    try {
      const rows = await vpsMonitorRepository.getHistory(hours);
  ...
  startMetricsCollection(): void {
    ...
    logger.info('Starting VPS metrics collection (every 1 minute)');
    ...
    setTimeout(() => {
      vpsMonitorRepository.cleanupOldMetrics();
  ...
  stopMetricsCollection(): void {
    if (this.collectionInterval) {
      ...
      logger.info('VPS metrics collection stopped');
    }
...
export const vpsMonitorService = new VPSMonitorService();
