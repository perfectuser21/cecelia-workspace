...
class WorkflowTrackerRepository {
  ...
  async findAllRuns(options: {
    ...
    const result = await db.query<WorkflowRun>(query, params);
    return result.rows;
  }

  async updateRun(runId: string, data: UpdateRunDTO): Promise<WorkflowRun | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await db.query<WorkflowRun>(query, values);
    return result.rows[0] || null;
  }

  async deleteRun(runId: string): Promise<boolean> {
    const result = await db.query('DELETE FROM workflow_runs WHERE run_id = $1', [runId]);
  ...
  async createEvent(runId: string, data: EmitEventDTO): Promise<WorkflowEvent> {
    const result = await db.query<WorkflowEvent>(
  ...
  async findLastEvent(runId: string): Promise<WorkflowEvent | null> {
    const result = await db.query<WorkflowEvent>(
      'SELECT * FROM workflow_events WHERE run_id = $1 ORDER BY created_at DESC LIMIT 1',
      [runId]
    );
    return result.rows[0] || null;
  }
  ...
  async countEvents(runId: string): Promise<number> {
    const result = await db.query<{ count: string }>(
      'SELECT COUNT(*) as count FROM workflow_events WHERE run_id = $1',
      [runId]
    );
    return parseInt(result.rows[0].count, 10);
  }
  ...
  async findStuckStartEvents(thresholdMs: number = 300000): Promise<WorkflowEvent[]> {
    const result = await db.query<WorkflowEvent>(
  ...
  async markSubstepStuck(runId: string, phase: Phase, substep: string): Promise<void> {
    await db.query(
      `INSERT INTO workflow_events (run_id, phase, substep, status, message)
       VALUES ($1, $2, $3, 'stuck', '超时 (>300s)')`,
      [runId, phase, substep]
    );

    await db.query(
      `UPDATE workflow_runs SET status = 'stuck', updated_at = NOW() WHERE run_id = $1`,
      [runId]
    );
  }
}

export const workflowTrackerRepository = new WorkflowTrackerRepository();
