...
export interface TaskRun {
  ...
  current_checkpoint?: string;  // 当前执行的 checkpoint ID (CP-001, CP-002, etc.)
  started_at: string;
  updated_at: string;
  completed_at?: string;
  error?: string;
  ...
  // Feature Registry 关联
  feature_id?: string;               // 关联的 Feature ID (F001)
  subproject_id?: string;            // 关联的 SubProject ID (SP001)
  // 实时追踪字段
...
export interface CheckpointProgress {
  run_id: string;
  checkpoint_id: string;
  name?: string;  // Checkpoint 名称（PRD task 描述）
  status: CheckpointStatus;
  started_at?: string;
  completed_at?: string;
  duration?: number;
...
export type SubProjectStatus = 'draft' | 'in_progress' | 'completed' | 'failed';

export interface Feature {
  id: string;                    // F001, F002, etc.
  name: string;                  // user-auth, checkout-flow (kebab-case)
  description: string;
  project: string;               // Cecelia-OS
