...
// 活跃任务持久化文件（放在 data 目录，容器挂载可写）
const DATA_DIR = process.env.DATA_DIR || '/app/data';
const ACTIVE_RUNS_FILE = process.env.ACTIVE_RUNS_FILE || `${DATA_DIR}/cecelia-active-runs.json`;
...
function parsePrdCheckpoints(content: string): CheckpointDetail[] {
  ...
  while ((match = cpRegex.exec(content)) !== null) {
    ...
    // 只保留第一次出现的（详细定义），跳过后面的引用
    if (checkpointMap.has(id)) continue;
    ...
    checkpointMap.set(id, { id, name, status });
  }

  // 按 ID 排序返回
  return Array.from(checkpointMap.values()).sort((a, b) => a.id.localeCompare(b.id));
}
...
// 保存活跃任务到文件
function saveActiveRuns(runs: Map<string, TaskRun>): void {
  try {
    const data = Array.from(runs.values()).filter((r) => r.status === 'running' || r.status === 'pending');
    fs.writeFileSync(ACTIVE_RUNS_FILE, JSON.stringify(data, null, 2));
  } catch (e) {
    console.error('Failed to save active runs:', e);
  }
}

// 从文件加载活跃任务
function loadActiveRuns(): TaskRun[] {
  try {
    if (fs.existsSync(ACTIVE_RUNS_FILE)) {
      const content = fs.readFileSync(ACTIVE_RUNS_FILE, 'utf-8');
      return JSON.parse(content);
    }
  } catch (e) {
...
class TaskTrackerService {
  ...
  constructor() {
    ...
    if (activeRuns.length > 0) {
      console.log(`Loaded ${activeRuns.length} active runs from file`);
    }
  }
  ...
  createRun(request: CreateRunRequest): TaskRun {
    const id = request.feature_branch || uuidv4();
    const now = new Date().toISOString();

    // 初始化 steps
    const steps = DEV_WORKFLOW_STEPS.map((s) => ({
      id: s.id,
      name: s.name,
    ...
    const run: TaskRun = {
      ...
      started_at: now,
      updated_at: now,
      mode: (request as any).mode || 'interactive',
      prd_title: (request as any).prd_title,
      prd_summary: (request as any).prd_summary,
      steps,
      current_step: 1,
    };
    ...
    // 持久化
    saveActiveRuns(this.runs);

    // 广播事件
    taskEvents.emit('task_created', { type: 'task_created', data: run });
  ...
  getCheckpoints(runId: string): CheckpointProgress[] {
    ...
    // 优先使用 PRD 解析的 checkpoints_detail
    if (run.checkpoints_detail && run.checkpoints_detail.length > 0) {
      return run.checkpoints_detail.map((cp, index) => ({
        run_id: runId,
        checkpoint_id: cp.id,
        name: cp.name,
        status: cp.status as 'pending' | 'in_progress' | 'done' | 'failed',
        started_at: run.started_at,
        completed_at: cp.status === 'done' ? run.updated_at : undefined,
      }));
    }

    // 如果没有 checkpoints_detail，尝试从 PRD 文件读取
    const prdPath = run.prd_path;
    if (prdPath) {
      try {
        // 尝试多个可能的路径
        const projectPath = `/home/xx/dev/${run.project}`;
        const possiblePaths = [
          path.join(projectPath, prdPath),
          path.join(projectPath, '.prd.md'),
          path.join(projectPath, '.dod.md'),
        ];

        for (const p of possiblePaths) {
          if (fs.existsSync(p)) {
            const content = fs.readFileSync(p, 'utf-8');
            const parsed = parsePrdCheckpoints(content);
            if (parsed.length > 0) {
  ...
  updateCheckpoint(
    ...
    // 广播事件
    taskEvents.emit('checkpoint_updated', { type: 'checkpoint_updated', data: { run, checkpoint } });
  ...
  updateRunStatus(
    ...
    // 广播事件
    taskEvents.emit('task_updated', { type: 'task_updated', data: run });
