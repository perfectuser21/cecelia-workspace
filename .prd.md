...
**成功标准**:
- `/api/brain/intent/parse` 接收 `{ input: string }` 返回 `{ intentType, confidence, entities, tasks }`
- 意图分类器区分 create_goal, create_project, create_task, query_status 等 9 种意图
- `intent/execute` 通过 handleAction 执行 action 意图，queryStatus handler 返回状态数据
- 置信度阈值防止低置信度自动执行
- 99 个测试全部通过
