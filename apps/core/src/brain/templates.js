...
/**
 * Full TRD Template Structure (8 sections matching TRD-TEMPLATE.md)
 */
const TRD_FULL_TEMPLATE = {
  name: 'trd-full',
  sections: [
    { id: 'overview', title: '概述', required: true },
    { id: 'architecture', title: '系统架构', required: true },
    { id: 'data_model', title: '数据模型', required: true },
    { id: 'prd_decomposition', title: 'PRD 拆解', required: true },
    { id: 'api_design', title: '接口设计', required: true },
    { id: 'tech_decisions', title: '技术决策', required: true },
    { id: 'acceptance_criteria', title: '验收标准', required: true },
...
/**
 * Generate a TRD from Goal + KR context (8-section structure)
 * @param {Object} params - Generation parameters
 ...
 * @param {string} [params.kr.title] - KR title
 * @param {number} [params.kr.progress] - KR progress (0-100)
 * @param {string} [params.kr.priority] - KR priority (P0/P1/P2)
 * @param {Object} [params.project] - Project context
 * @param {string} [params.project.name] - Project name
 * @param {string} [params.project.repo_path] - Repository path
 * @param {Array<{title: string, description?: string}>} [params.milestones] - Milestones for PRD decomposition
 * @returns {string} Generated TRD markdown with 8 sections
 */
function generateTrdFromGoalKR(params) {
  ...
  trd += `Auto-generated by TRD Template Engine\n\n`;
  trd += `### 1.3 范围\n\n`;
  trd += `- **包含**：${title}\n`;
  trd += `- **不包含**：无关模块的修改\n\n`;
  ...
  if (milestones.length > 0) {
    milestones.forEach((m, i) => {
      trd += `| ${m.title} | ${m.description || m.title} | ${i === 0 ? '无' : milestones[i - 1].title} |\n`;
    });
  } else {
    trd += `| ${title} | 核心模块 | 无 |\n`;
  }
  ...
  if (milestones.length > 0) {
    ...
    milestones.forEach((m, i) => {
      const prefix = i === 0 ? '' : '    ↓\n';
      trd += `${prefix}PRD-${String(i + 1).padStart(2, '0')} (${m.title})\n`;
    });
    trd += '```\n\n';
    ...
    milestones.forEach((m, i) => {
      ...
      trd += `| ${String(i + 1).padStart(2, '0')} | ${m.title} | ${dep} | 中 |\n`;
    });
  } else {
    trd += `待定义（需提供 milestones）。\n`;
  }
  trd += '\n';
...
function listTemplates() {
  return [
    ...
    { name: 'trd-full', sectionCount: TRD_FULL_TEMPLATE.sections.length }
  ];
}
