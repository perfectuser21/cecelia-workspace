...
describe('Templates Module', () => {
  ...
  describe('validatePrd', () => {
    const completePrd = `## PRD - Test Feature

**需求来源**: KR2 任务
**功能描述**: 添加验证层
**成功标准**:
- [ ] validatePrd 返回 valid
- [ ] API 返回 200
**非目标**:
- 不做 AI 评估`;
    ...
    it('returns invalid for missing success criteria', () => {
      const prd = `## PRD - Test
**需求来源**: test
**功能描述**: test
**非目标**: none`;
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('成功标准'))).toBe(true);
    });

    it('returns invalid for missing non-goal section', () => {
      const prd = `## PRD - Test
**需求来源**: test
**功能描述**: test
**成功标准**:
- [ ] item`;
      const result = validatePrd(prd);
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('非目标'))).toBe(true);
    });

    it('detects fuzzy words in success criteria and lowers score', () => {
      const prd = `## PRD - Test
**需求来源**: test
**功能描述**: test
**成功标准**:
...
**非目标**: none`;
      ...
      expect(result.errors.some(e => e.includes('优化'))).toBe(true);
    });

    it('warns about missing checkboxes in success criteria', () => {
      const prd = `## PRD - Test
**需求来源**: test
**功能描述**: test
**成功标准**:
...
**非目标**: none`;
      const result = validatePrd(prd);
      expect(result.errors.some(e => e.includes('checkable'))).toBe(true);
    });

    it('validates PRDs generated by generatePrdFromGoalKR (includes all sections)', () => {
      const prd = generatePrdFromGoalKR({
        title: 'Test feature',
        description: 'A test feature',
        kr: { title: 'KR1', progress: 50, priority: 'P0' },
        project: { name: 'test-project', repo_path: '/test' }
      });
      const result = validatePrd(prd);
      expect(result.valid).toBe(true);
      expect(result.score).toBeGreaterThanOrEqual(80);
    ...
    it('renderPrd output missing non-goal section is flagged', () => {
      const prd = renderPrd({
        projectName: 'test',
      ...
      const result = validatePrd(prd);
      // renderPrd does not include 非目标 section
      expect(result.errors.some(e => e.includes('非目标'))).toBe(true);
    });
  });
...
some content`;
    ...
    it('returns invalid for TRD missing all sections', () => {
      const result = validateTrd('just plain text');
      expect(result.valid).toBe(false);
      expect(result.score).toBe(0);
