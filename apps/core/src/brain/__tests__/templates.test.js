...
describe('Templates Module', () => {
  ...
  describe('validatePrd', () => {
    ...
    it('returns score 0 for null/undefined', () => {
      expect(validatePrd(null).score).toBe(0);
      expect(validatePrd(undefined).score).toBe(0);
    });

    it('validates a complete PRD generated by renderPrd', () => {
      const prd = renderPrd({
        projectName: 'test-feature',
        intentType: 'create_feature',
        tasks: [{ title: 'Task 1', description: 'Do thing' }],
        originalInput: 'Add test feature',
        entities: {}
      });
    ...
    it('validates a complete PRD generated by generatePrdFromGoalKR', () => {
      const prd = generatePrdFromGoalKR({
        title: 'Test Feature',
        description: 'Implement test feature',
        kr: { title: 'KR1', progress: 50, priority: 'P0' },
        project: { name: 'test', repo_path: '/test' }
      });
      const result = validatePrd(prd);
      expect(result.score).toBeGreaterThanOrEqual(60);
    });
    ...
    it('detects missing sections', () => {
      const result = validatePrd('# Just a title\n\nSome content');
      expect(result.valid).toBe(false);
      expect(result.missing_fields.length).toBeGreaterThan(0);
    });

    it('detects fuzzy words in acceptance criteria', () => {
      const prd = `---\nid: test\nversion: 1.0.0\n---\n## 背景\ntest\n## 功能描述\ntest\n## 成功标准\n- [ ] 优化性能\n## 涉及文件\n- test.js\n## 非目标\n- none`;
      const result = validatePrd(prd);
      expect(result.missing_fields).toContain('criteria_has_fuzzy_words');
    });
  });
  ...
  describe('validateTrd', () => {
    ...
    it('returns score 0 for null/undefined', () => {
      expect(validateTrd(null).score).toBe(0);
      expect(validateTrd(undefined).score).toBe(0);
    });

    it('validates a complete TRD generated by renderTrd', () => {
      const trd = renderTrd({
        projectName: 'test-service',
        intentType: 'create_project',
        tasks: [{ title: 'Task 1', description: 'Do thing', priority: 'P0' }],
        originalInput: 'Build test service',
        entities: {}
      });
    ...
    it('validates a complete TRD generated by generateTrdFromGoal', () => {
      const trd = generateTrdFromGoal({
        title: 'Test Goal',
        description: 'Implement test goal',
    ...
    it('detects missing sections', () => {
      const result = validateTrd('# Just a title\n\nSome content');
      expect(result.valid).toBe(false);
      expect(result.missing_fields.length).toBeGreaterThan(0);
    });
    ...
    it('reports specific missing sections', () => {
      const trd = `## 技术背景\ntest\n## 架构设计\ntest`;
      const result = validateTrd(trd);
      expect(result.missing_fields).toContain('api_design');
      expect(result.missing_fields).toContain('data_model');
      expect(result.missing_fields).toContain('test_strategy');
    });

    it('gives bonus points for code blocks', () => {
      const withCode = `## 技术背景\ntest\n## 架构设计\ntest\n## API 设计\ntest\n## 数据模型\ntest\n## 测试策略\ntest\n## 实施计划\ntest\n\`\`\`js\ncode\n\`\`\``;
      const withoutCode = `## 技术背景\ntest\n## 架构设计\ntest\n## API 设计\ntest\n## 数据模型\ntest\n## 测试策略\ntest\n## 实施计划\ntest`;
      expect(validateTrd(withCode).score).toBeGreaterThan(validateTrd(withoutCode).score);
    });
  });
});
