...
describe('Templates Module', () => {
  ...
  describe('listTemplates', () => {
    it('returns list of available templates', () => {
      ...
      expect(templates).toContainEqual({ name: 'trd-full', sectionCount: 8 });
    });
  });
...
`;
  ...
  describe('TRD_FULL_TEMPLATE', () => {
    it('has 8 sections', () => {
      expect(TRD_FULL_TEMPLATE.sections).toHaveLength(8);
    });

    it('has all required section ids', () => {
      const ids = TRD_FULL_TEMPLATE.sections.map(s => s.id);
      expect(ids).toContain('overview');
      expect(ids).toContain('architecture');
      expect(ids).toContain('data_model');
      expect(ids).toContain('prd_decomposition');
      expect(ids).toContain('api_design');
      expect(ids).toContain('tech_decisions');
      expect(ids).toContain('acceptance_criteria');
      expect(ids).toContain('appendix');
    });

    it('is retrievable via getTemplate', () => {
      const tmpl = getTemplate('trd-full');
      expect(tmpl).not.toBeNull();
      expect(tmpl.name).toBe('trd-full');
    });

    it('appears in listTemplates', () => {
      const list = listTemplates();
      const full = list.find(t => t.name === 'trd-full');
      expect(full).toBeDefined();
      expect(full.sectionCount).toBe(8);
    });
  });

  describe('generateTrdFromGoalKR', () => {
    it('generates TRD with title only', () => {
      const trd = generateTrdFromGoalKR({ title: 'Auth System' });
      expect(trd).toContain('# TRD: Auth System');
      expect(trd).toContain('## 1. 概述');
      expect(trd).toContain('## 2. 系统架构');
      expect(trd).toContain('## 3. 数据模型');
      expect(trd).toContain('## 4. PRD 拆解');
      expect(trd).toContain('## 5. 接口设计');
      expect(trd).toContain('## 6. 技术决策');
      expect(trd).toContain('## 7. 验收标准');
      expect(trd).toContain('## 8. 附录');
    });

    it('includes KR context', () => {
      const trd = generateTrdFromGoalKR({
        title: 'Feature X',
        kr: { title: 'KR2: Standardization', progress: 40, priority: 'P0' }
      });
      expect(trd).toContain('KR: KR2: Standardization');
      expect(trd).toContain('progress: 40%');
      expect(trd).toContain('priority: P0');
    });
    ...
    it('generates PRD dependency graph from milestones', () => {
      const trd = generateTrdFromGoalKR({
        title: 'OKR System',
        milestones: [
          { title: 'Data Model', description: 'DB schema' },
          { title: 'API Layer', description: 'REST endpoints' },
          { title: 'Frontend', description: 'UI components' }
        ]
      });
      expect(trd).toContain('PRD-01 (Data Model)');
      expect(trd).toContain('PRD-02 (API Layer)');
      expect(trd).toContain('PRD-03 (Frontend)');
      expect(trd).toContain('| 01 | Data Model | 无 | 中 |');
      expect(trd).toContain('| 02 | API Layer | 01 | 中 |');
      expect(trd).toContain('| 03 | Frontend | 02 | 中 |');
    });

    it('includes frontmatter', () => {
      const trd = generateTrdFromGoalKR({ title: 'Test' });
      expect(trd).toContain('---');
      expect(trd).toContain('id: trd-auto-test');
    ...
    it('contains all 8 numbered sections', () => {
      const trd = generateTrdFromGoalKR({
        title: 'Valid TRD',
        description: 'A complete TRD',
        milestones: [{ title: 'Phase 1' }]
      });
      for (let i = 1; i <= 8; i++) {
        expect(trd).toContain(`## ${i}.`);
      }
    });

    it('handles missing optional params gracefully', () => {
      const trd = generateTrdFromGoalKR({ title: 'Minimal' });
      expect(trd).toContain('Auto-generated by TRD Template Engine');
      expect(trd).toContain('待定义（需提供 milestones）');
    });

    it('includes milestone acceptance criteria', () => {
      const trd = generateTrdFromGoalKR({
        title: 'Test',
        milestones: [
          { title: 'Step A' },
      ...
      expect(trd).toContain('- [ ] Step A 完成');
      expect(trd).toContain('- [ ] Step B 完成');
    });
  });
});
