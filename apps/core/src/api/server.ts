/**
 * Core API Server
 * Serves frontend and provides REST API endpoints
...
// Proxy /semantic-brain to Semantic Brain API (Task Intelligence)
app.use('/semantic-brain', createProxyMiddleware({
  target: QUALITY_API,  // Same host as quality API (port 5220)
  changeOrigin: true,
  pathRewrite: { '^/semantic-brain': '' }  // /semantic-brain/plan → /plan
}));
...
// Task System API routes (Task Management with PostgreSQL)
app.use('/api/tasks', taskSystemRoutes);

// Brain API routes (Orchestrator + Perception)
