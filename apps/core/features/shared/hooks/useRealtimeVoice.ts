...
export function useRealtimeVoice(options: UseRealtimeVoiceOptions = {}): UseRealtimeVoiceReturn {
  ...
  const interruptPlayback = useCallback(() => {
    ...
    setIsPlaying(false);
    // Cancel any pending response.create
  ...
  const handleToolCall = useCallback(async (callId: string, toolName: string, argsJson: string) => {
    try {
      ...
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        ...
        // Mark that we're waiting to create a response
        pendingResponseCreateRef.current = true;

        // Wait for any active response to finish, then create new response
        const waitForResponseDone = () => {
          return new Promise<void>((resolve) => {
            const checkInterval = setInterval(() => {
              // Check if cancelled by user interrupt or response finished
              if (!pendingResponseCreateRef.current || !currentResponseIdRef.current) {
        ...
        // Wait for current response to finish, then audio playback, then create new response
        waitForResponseDone().then(() => {
          // If cancelled by user interrupt, don't create response
          if (!pendingResponseCreateRef.current) {
            console.log('[Realtime] Response creation cancelled by user interrupt');
            return;
          }
          ...
          setTimeout(() => {
            // Final check: no active response and not cancelled
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN
                && !currentResponseIdRef.current && pendingResponseCreateRef.current) {
              pendingResponseCreateRef.current = false;
              wsRef.current.send(JSON.stringify({
