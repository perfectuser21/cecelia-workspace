...
export default function SessionMonitor() {
  ...
  if (loading && !data) {
    return (
      <div className="p-6">
        <div className="animate-pulse">Loading session status...</div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4 mt-1">
            ...
            {data?.last_check && (
              <span className="text-sm text-gray-500 dark:text-gray-400">
                上次检查: {new Date(data.last_check).toLocaleString('zh-CN')}
              </span>
            )}
          </div>
        </div>
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`px-2.5 py-1 rounded-lg text-sm font-medium ${getStatusColor(platform.status)}`}>
                {getStatusIcon(platform.status)} {getStatusText(platform.status)}
              </span>
            </div>
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              ...
              <p>
                检查时间: {new Date(platform.lastCheck).toLocaleString('zh-CN')}
              </p>
            </div>
          </div>
        ))}
