  ...
  return (
    <div className={`min-h-screen transition-colors ${
      ...
      <main className={isAuthenticated ? `${collapsed ? 'ml-16' : 'ml-64'} pt-16 transition-all duration-300` : ""}>
        <div className={isAuthenticated && !['/canvas', '/voice-canvas', '/canvas-flow', '/canvas-konva', '/test-canvas', '/simple-test', '/whiteboard', '/project-panorama', '/viz-dashboard'].includes(location.pathname) ? "p-8" : ""} style={['/canvas', '/voice-canvas', '/canvas-flow', '/canvas-konva', '/test-canvas', '/simple-test', '/whiteboard', '/project-panorama', '/viz-dashboard'].includes(location.pathname) ? { position: 'absolute', top: 64, left: collapsed ? 64 : 256, right: 0, bottom: 0 } : {}}>
          <DynamicRouter>
            {/* 登录页面（静态路由） */}
