...
router.get('/dashboard', async (req, res, next) => {
    try {
        ...
        const dashboard = await metrics_service_1.metricService.getDashboardMetrics(timeRange);
        res.json(dashboard);
    }
    catch (error) {
        next(error);
...
router.get('/latest/:accountId', async (req, res, next) => {
    try {
        ...
        const metric = await metrics_service_1.metricService.getLatestMetric(accountId);
        if (!metric) {
...
router.get('/account/:accountId', async (req, res, next) => {
    try {
        ...
        const metrics = await metrics_service_1.metricService.getMetricsByAccount(accountId, startDate, endDate);
        res.json(metrics);
    }
    catch (error) {
        next(error);
...
router.get('/date/:date', async (req, res, next) => {
    try {
        ...
        const metrics = await metrics_service_1.metricService.getMetricsByDate(date);
        res.json(metrics);
    }
    catch (error) {
        next(error);
...
router.get('/platform/:platform', async (req, res, next) => {
    try {
        ...
        const metrics = await metrics_service_1.metricService.getMetricsByPlatform(platform, startDate, endDate);
        res.json(metrics);
    }
    catch (error) {
        next(error);
...
router.get('/range', async (req, res, next) => {
    try {
        ...
        const metrics = await metrics_service_1.metricService.getMetricsByDateRange(startDate, endDate);
        res.json(metrics);
    }
    catch (error) {
        next(error);
...
router.post('/report_daily', async (req, res, next) => {
    try {
        ...
        const report = await metrics_service_1.metricService.generateDailyReport(date);
        res.json(report);
    }
    catch (error) {
        next(error);
...
router.get('/reports/date/:date', async (req, res, next) => {
    try {
        const { date } = req.params;
        const report = await metrics_service_1.metricService.getReportByDate(date);
        if (!report) {
...
router.get('/reports/recent', async (req, res, next) => {
    try {
        ...
        const reports = await metrics_service_1.metricService.getRecentReports(limit);
        res.json(reports);
    }
    catch (error) {
        next(error);
