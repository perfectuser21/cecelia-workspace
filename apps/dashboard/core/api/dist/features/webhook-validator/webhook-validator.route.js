...
const webhookValidator = new webhook_validator_service_1.WebhookValidatorService();
/**
...
router.get('/status', async (req, res) => {
    try {
        ...
        logger_1.default.error('[WebhookValidator] Failed to generate status report', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
...
router.post('/test', async (req, res) => {
    try {
        ...
        logger_1.default.error('[WebhookValidator] Test failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
...
router.post('/test-execution', async (req, res) => {
    try {
        ...
        const result = await webhookValidator.sendTestExecutionCallback(taskId, runId);
        res.json({
            success: true,
            data: result,
        });
        ...
        logger_1.default.error('[WebhookValidator] Test execution failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
...
router.post('/test-retry', async (req, res) => {
    try {
        ...
        logger_1.default.error('[WebhookValidator] Retry test failed', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
...
router.post('/generate-report', async (req, res) => {
    try {
        ...
        // 检查运行目录是否存在
        try {
            await promises_1.default.access(runDir);
        }
        catch {
        ...
        const statusFile = path_1.default.join(runDir, 'webhook-status.json');
        await promises_1.default.writeFile(statusFile, JSON.stringify(statusReport, null, 2));
        // 生成 Markdown 报告
        const markdownReport = generateMarkdownReport(statusReport);
        const reportFile = path_1.default.join(runDir, 'logs', 'webhook-test-report.md');
        // 确保 logs 目录存在
        await promises_1.default.mkdir(path_1.default.join(runDir, 'logs'), { recursive: true });
        await promises_1.default.writeFile(reportFile, markdownReport);
        logger_1.default.info('[WebhookValidator] Report generated', {
            runId,
            statusFile,
            reportFile,
        });
        ...
        logger_1.default.error('[WebhookValidator] Failed to generate report', error);
        res.status(500).json({
            success: false,
            error: error.message,
        });
...
//# sourceMappingURL=webhook-validator.route.js.map