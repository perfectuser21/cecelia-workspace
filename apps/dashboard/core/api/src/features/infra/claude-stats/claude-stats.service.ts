...
// Cache configuration - keyed by days parameter
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minute cache
const statsCache: Map<number, { data: StatsResponse; timestamp: number }> = new Map();

// Module initialization log
logger.info('[claude-stats] Service module loaded with caching enabled');
...
export async function getStats(days: number = 30): Promise<StatsResponse> {
  logger.info(`[claude-stats] getStats called with days=${days}`);

  // Check cache
  const now = Date.now();
  const cached = statsCache.get(days);
  logger.info(`[claude-stats] Cache check: cached=${!!cached}, age=${cached ? now - cached.timestamp : 'N/A'}ms, TTL=${CACHE_TTL_MS}ms`);

  if (cached && (now - cached.timestamp) < CACHE_TTL_MS) {
    logger.info(`[claude-stats] Returning cached data for ${days} days`);
    return cached.data;
  }

  logger.info(`[claude-stats] Cache miss/expired for ${days} days, recalculating...`);
  ...
  // Update cache
  statsCache.set(days, { data: result, timestamp: Date.now() });
  logger.info(`[claude-stats] Calculation complete for ${days} days, took ${Date.now() - startTime}ms, cached`);
