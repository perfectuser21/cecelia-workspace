...
# 确定项目目录
# ============================================================
PROJECT_PATH="${PROJECT_DIR:-/home/xx/dev/zenithjoy-autopilot/workflows}"
...
cd "$PROJECT_PATH" || cd /home/xx/dev/zenithjoy-autopilot || exit 1
...
# 检查 Claude 输出中是否提到 workflow ID
WORKFLOW_ID=""
if [[ -f "$WORK_DIR/claude_output.txt" ]]; then
  # 尝试从输出中提取 workflow ID（格式如 "workflow ID: xxx" 或 "created workflow xxx"）
  WORKFLOW_ID=$(grep -oE '[a-zA-Z0-9]{16,}' "$WORK_DIR/claude_output.txt" | head -1 || echo "")
fi

log_info "执行完成"
[[ -n "$WORKFLOW_ID" ]] && log_info "检测到 Workflow ID: $WORKFLOW_ID"
